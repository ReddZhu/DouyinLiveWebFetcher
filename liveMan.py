...
# 增加了携带网页版抖音的用户的信息进行信息抓取，而不是匿名抓取
# 自动从浏览器获取 Cookie 的库（此处以 Chrome 为例）
...
def cookies_to_header(cookiejar):
    """
    将 CookieJar 转换成 HTTP 请求中的 Cookie 字符串
    """
    return "; ".join([f"{cookie.name}={cookie.value}" for cookie in cookiejar])
...
class DouyinLiveWebFetcher:
    def __init__(self, live_id):
        """
        直播间弹幕抓取对象
        :param live_id: 直播间的直播id，如：https://live.douyin.com/261378947940 中的261378947940
        """
        ...
        # 自动加载浏览器中登录后的 Cookie（此处以 Chrome 为例）
        self.browser_cookies = browser_cookie3.chrome(domain_name="douyin.com")
        # 尝试从加载的 Cookie 中获取 ttwid
        self.__ttwid = get_cookie_value(self.browser_cookies, "ttwid")
        self.__room_id = None
    ...
    def ttwid(self):
        """
        返回 Cookie 中的 ttwid 值
        """
        if self.__ttwid:
            return self.__ttwid
        # 如果没有，则尝试请求首页（一般情况下，浏览器Cookie中应已包含登录信息）
        headers = {
            "User-Agent": self.user_agent,
        }
        try:
            response = requests.get(self.live_url, headers=headers, cookies=self.browser_cookies)
            response.raise_for_status()
        except Exception as err:
            print("【X】Request the live url error: ", err)
            return None
        else:
            self.__ttwid = get_cookie_value(response.cookies, "ttwid") or get_cookie_value(self.browser_cookies, "ttwid")
            return self.__ttwid

    @property
    def room_id(self):
        """
        根据直播间的地址获取真正的直播间roomId
        """
        if self.__room_id:
        ...
        try:
            response = requests.get(url, headers=headers, cookies=self.browser_cookies)
            response.raise_for_status()
        except Exception as err:
    ...
    def _connectWebSocket(self):
        """
        连接抖音直播间 WebSocket 服务器，自动使用浏览器 Cookie 发起请求
        """
        ...
        if signature:
            ...
            print("【X】Failed to generate signature.")
            return

        headers = {
            "cookie": cookies_to_header(self.browser_cookies),
            "user-agent": self.user_agent,
        }
